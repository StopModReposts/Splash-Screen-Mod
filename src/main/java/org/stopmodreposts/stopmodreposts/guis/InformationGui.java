...
public class InformationGui extends GuiScreen {
	...
	private static final ResourceLocation SMR_LOGO = new ResourceLocation(References.MODID, "textures/guis/smr_logo.png");
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
		if(button == this.btnClose) {
	    	...
	    	try {
	    		 ...
	    		 */
				References.STOP_REPOST_FILE.createNewFile();
			} catch (IOException e) {
				e.printStackTrace();
			}
		}else if(button == this.btnMoreInfo) {
			ITextComponent linkHelper = new TextComponentString("");
			linkHelper.getStyle().setClickEvent(new ClickEvent( ClickEvent.Action.OPEN_URL, References.URL));
